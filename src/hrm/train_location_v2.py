...
Uses larger model, better regularization, and improved training strategies.
"""
...
def train_location_model(
    ...
    max_seq_len: int = 50,
    hidden_size: int = 384,  # Increased from 256
    num_layers: int = 6,  # Increased from 4
    num_heads: int = 6,  # Increased from 4
    expansion: float = 4.0,
    high_level_cycles: int = 3,  # Increased from 2
    low_level_cycles: int = 3,  # Increased from 2
    use_features: bool = True,
    batch_size: int = 96,  # Decreased for larger model
    learning_rate: float = 5e-5,  # Lower LR for more stable training
    num_epochs: int = 150,  # More epochs
    save_every: int = 5,
    device: str = "cuda",
):
    """
    ...
    # Warm up for first 10 epochs, then cosine decay
    warmup_epochs = 10
    def lr_lambda(epoch):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if test_acc1 >= 0.40:
            ...
            print(f"{'='*60}\n")
            # Continue training to potentially reach even higher accuracy
