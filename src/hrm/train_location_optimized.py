"""
Optimized training script for HRM model on next location prediction.
...
def train_location_model(
    ...
    num_heads: int = 4,
    expansion: float = 2.0,   # Reduced from 4.0
    high_level_cycles: int = 2,
    low_level_cycles: int = 1,  # Reduced from 2
    use_features: bool = True,
    batch_size: int = 256,    # Increased from 128
    learning_rate: float = 3e-4,  # Increased
    num_epochs: int = 150,    # Increased
    save_every: int = 5,
    device: str = "cuda",
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % save_every == 0 or val_acc1 > best_val_acc:
            checkpoint_path = f"{checkpoint_dir}/location_hrm_opt_epoch{epoch}.pt"
            torch.save({
                'epoch': epoch,
        ...
        if val_acc1 > best_val_acc:
            ...
            no_improve_count = 0
            best_checkpoint_path = f"{checkpoint_dir}/location_hrm_opt_best_val.pt"
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
        ...
        if test_acc1 > best_test_acc:
            ...
            best_test_checkpoint_path = f"{checkpoint_dir}/location_hrm_opt_best_test.pt"
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
